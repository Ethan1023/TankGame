...
bool process(std::vector<double>* xPath, std::vector<double>* yPath){
...
//	auto t1 = Clock::now();
    ...
    for(i;i<length;i++){
        ...
        //smallest is -1 when supplied coordinates are all undefined (called by circlular getShape() tank
        if(smallest==-1){
...
bool collision(int index){
    for(int i=0;i<tanks.size();i++){
        if(i==index || tanks[i]->getCurrentRoom()!=tanks[index]->getCurrentRoom())
            ...
            if(checkcollision(tanks[index]->getShape(), tanks[i]->getShape(), tanks[index]->getX()-tanks[index]->getOldX(), tanks[index]->getY()-tanks[index]->getOldY())){//, gradient, setgradient))
